...
class Forfieted extends StatefulWidget {
  const Forfieted({Key key}) : super(key: key);

  @override
  _ForfietedState createState() => _ForfietedState();
}

class _ForfietedState extends State<Forfieted> {
  final ForfietedList = [
    {
      'name': 'XRP',
      'shortName': 'XRP',
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      itemCount: ForfietedList.length,
      physics: BouncingScrollPhysics(),
      itemBuilder: (context, index) {
        final item = ForfietedList[index];
        return Padding(
          padding: (index != ForfietedList.length - 1)
              ? EdgeInsets.fromLTRB(
